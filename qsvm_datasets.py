...
def ad_hoc_data(training_size, test_size, n, gap, PLOT_DATA):
    ...
    if n == 2:
        ...
        if PLOT_DATA:
            ...
            f, ax = plt.subplots(1,2, figsize=(18,8))  # figsize=(8,3.75)
            ax[0].imshow(np.asmatrix(sample_Total).T, interpolation='nearest', origin='lower', cmap='PiYG', extent=[0, 2*np.pi, 0, 2*np.pi]) # 'RdYlBu'
            
            # colors = ['b','r']
            colors = ['#448f47', '#983d72']
            for k in range(0, 2):
                ax[1].scatter(sample_train[label_train == k, 0][:training_size], sample_train[label_train == k, 1][:training_size], c = colors[k], s=80)
